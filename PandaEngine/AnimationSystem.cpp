...
void AnimationSystem::UpdateBoneTransforms(Animation* animation, float dt)
{
	if (animation->mesh->useBone)
	{
		sModelDrawInfo modelInfo;
		meshManager->GetModelDrawInfo(animation->mesh->friendlyName, modelInfo);
		Node* rootNode = modelInfo.RootNode;
		CalculateMatrices(rootNode, glm::mat4(1.0f), modelInfo);
	}
}

void AnimationSystem::CalculateMatrices(Node* node, const glm::mat4& parentTransformationMatrix, 
	sModelDrawInfo& modelInfo)
{
